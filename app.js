...
const STRINGS = {
    en: {
        ...
        howToInterpretHtml1: "A polarizing filter isn't a shutter that simply blocks or passes light. It splits any incoming light into two perpendicular components &mdash; one aligned with its own transmission axis, one at 90&deg; to it &mdash; and lets only the aligned component through, absorbing the rest. If the light's polarization sits at angle &Delta;&theta; from that axis, the transmitted fraction is <strong>cos&sup2;(&Delta;&theta;)</strong> (Malus's law). Light that has just passed a filter emerges fully aligned with that filter's axis, so the next filter measures its own &Delta;&theta; from there &mdash; not from some fixed zero.",
        howToInterpretHtml2: "That's why two crossed filters (&Delta;&theta;=90&deg;) block almost everything: cos&sup2;(90&deg;)=0. But insert a third filter at 45&deg; between them, and the light gets decomposed twice instead of once &mdash; each 45&deg; step only costs a <strong>cos&sup2;(45&deg;)=50%</strong> loss, letting about 12.5% back through. Try the <strong>Three-filter reveal</strong> preset, then compare it with <strong>Gentle rotation</strong>, which spreads the same 0&deg;&rarr;90&deg; turn over five small steps and lets even more light through.",
    ...
    es: {
        ...
        howToInterpretHtml1: 'Un filtro polarizador no es un obturador que simplemente bloquea o deja pasar la luz. Descompone cualquier luz que le llega en dos componentes perpendiculares &mdash; una alineada con su propio eje de transmisión y otra a 90&deg; de este &mdash; y solo deja pasar la componente alineada, absorbiendo el resto. Si la polarización de la luz está a un ángulo &Delta;&theta; de ese eje, la fracción transmitida es <strong>cos&sup2;(&Delta;&theta;)</strong> (ley de Malus). Nada más atravesar un filtro, la luz queda totalmente alineada con el eje de ese filtro, así que el siguiente filtro mide su propio &Delta;&theta; desde ahí, no desde un cero fijo.',
        howToInterpretHtml2: 'Por eso dos filtros cruzados (&Delta;&theta;=90&deg;) bloquean casi todo: cos&sup2;(90&deg;)=0. Pero si colocas un tercer filtro a 45&deg; entre medias, la luz se descompone dos veces en lugar de una &mdash; cada paso de 45&deg; solo cuesta una pérdida de <strong>cos&sup2;(45&deg;)=50%</strong>, dejando pasar de nuevo cerca del 12,5%. Prueba el preajuste <strong>Revelación de tres filtros</strong> y compáralo con <strong>Giro suave</strong>, que reparte ese mismo giro de 0&deg; a 90&deg; en cinco pasos pequeños y deja pasar todavía más luz.',
    ...
    ja: {
        ...
        howToInterpretHtml1: '偏光フィルターは、光を単純に通す・通さないというシャッターではありません。入ってくる光を、フィルター自身の透過軸に沿った成分と、それに垂直な成分という2つの直交成分に分解し、軸に沿った成分だけを通過させ、残りは吸収します。光の偏光方向がその軸から角度&Delta;&theta;だけずれている場合、透過する割合は<strong>cos&sup2;(&Delta;&theta;)</strong>になります(マリュスの法則)。フィルターを通過した直後の光は、そのフィルターの軸に完全に揃うため、次のフィルターは固定の基準ではなく、その軸を基準に&Delta;&theta;を測ります。',
        howToInterpretHtml2: 'そのため、90&deg;で交差した2枚のフィルターはほぼすべての光を遮断します(cos&sup2;(90&deg;)=0)。しかし、その間に45&deg;のフィルターを1枚挟むと、光は1回ではなく2回分解されることになり &mdash; 45&deg;の各段階で失われるのは<strong>cos&sup2;(45&deg;)=50%</strong>だけなので、約12.5%の光が戻ってきます。<strong>3枚フィルターの復活</strong>を試したあと、同じ0&deg;から90&deg;への回転を5つの小さな段階に分ける<strong>緩やかな回転</strong>と比べてみてください。さらに多くの光が透過します。',
